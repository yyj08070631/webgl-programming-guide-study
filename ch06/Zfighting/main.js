...
function main () {
    ...
    function start () {
        ...
        function keydown (e, gl, n, u_ViewMatrix, viewMatrix, u_ModelMatrix, modelMatrix) {
            const updateX = deg => Math.sin(deg * Math.PI / 180) * 15
            const updateZ = deg => Math.cos(deg * Math.PI / 180) * 15
            const map = {
                39: () => { currDeg += 1; g_eyeX = updateX(currDeg); g_eyeZ = updateZ(currDeg) }, // 方向键右
                37: () => { currDeg -= 1; g_eyeX = updateX(currDeg); g_eyeZ = updateZ(currDeg) }, // 方向键左
                // 38: () => { g_eyeY += 0.1 }, // 方向键上
                // 40: () => { g_eyeY -= 0.1 }, // 方向键下
            }
    ...
    function initVertexBuffers (gl, a_Position, a_Color) {
        const verticesAndSizes = new Float32Array([
            ...
            -2.5, -2.5,  -5.0,  0.4,  1.0,  0.4,
             ...
             0.0,  3.0,  -3.0,  1.0,  0.4,  0.4, // The yellow triagle
            -3.0, -3.0,  -3.0,  1.0,  1.0,  0.4,
             3.0, -3.0,  -3.0,  1.0,  1.0,  0.4, 
        ])
